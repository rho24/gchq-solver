...
let getGroups (axis) =
  foldi (fun index acc elem -> 
    match elem with
    | W -> match List.length acc.CurrentGroup with
                            | 0 -> acc
...
let printResultGrid (grid) (rowTests : int list list) (columnTests : int list list) =
  ...
  fprintfn output ""
  fprintf output "   "
  for i in 1..27 do fprintf output "\u2588\u2588"
  fprintfn output ""
...
let addRandomGuess grid =
  let x = rnd.Next(0, 25)
  let y = rnd.Next(0, 25)
  let z = if rnd.Next(0, 2) = 0 then B else W
...
let rec getPossibles axisLength testGroups =
  ...
  | group::rest, _ ->
    ...
    List.collect (fun x -> x) [for possible in groupPossibles ->
      if possible.Length = axisLength then [possible]
      else [
        for p in getPossibles (axisLength - possible.Length - 1) rest ->
          possible @ (List.replicate 1 W) @ p
      ]
    ]
...
let trySolveAxis axis (testGroups: int list) =
  ...
  let result = 
    ...
    | _ -> 
      ...
      | overlap ->
        let possibles = 
          getPossibles axis.Length testGroups
